    ...
    for url in urls:
        try:
            ...
            if parts[2] == "twitter.com":
                ...
                if (bs4.BeautifulSoup(urlopen("http://data.alexa.com/data?cli=10&dat=s&url=" + url).read(), "xml").find(
                        "REACH")):
                    rank = \
                        bs4.BeautifulSoup(urlopen("http://data.alexa.com/data?cli=10&dat=s&url=" + url).read(),
                                          "xml").find("REACH")['RANK']
...
def rank_url_adult_wot(dataset):
    ...
    for data in dataset:
        ...
        mal = False
        total_wot = total + 1
        [counter, total, adult, mal, counter_wot, total_wot] = ranking(urls, counter, total, mal, counter_wot,
                                                                       total_wot)
        if adult == 10:
            adult_global = 10
